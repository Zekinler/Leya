...
module.exports = {
	...
	async execute(interaction, db, client) {
		...
		const timedout = async () => {
			...
			await interaction.followUp({ content: 'Input timed out; Level Reward removal cancelled', ephemeral: true });
		};

		await interaction.followUp({ content: 'Send the level of the level reward you to remove', ephemeral: true });
		...
		const allowedValues = await databaseGuild.settings.levelingSettings.levelRewards.map((levelReward) => levelReward.level);
		...
		client.messageInputHandlers.push(
			new MessageInputHandler(interaction.member.id, interaction.channelId, interaction.guildId,
				...
				async (input) => {
					guildLevelingSettings.levelRewards.splice(await guildLevelingSettings.levelRewards.findIndex((levelReward) => levelReward.level === input), 1);
					...
					await mainInteraction.delete();
					await interaction.followUp({ embeds: [embed], components: [row] });
					await interaction.followUp({ content: 'Successfully removed the level reward', ephemeral: true });
				},
				timedout,
			),
		);
