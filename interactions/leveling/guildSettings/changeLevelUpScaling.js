...
module.exports = {
	...
	async execute(interaction, db, client) {
		...
		await interaction.followUp({ content: 'Send a round number between 1-50 to be the Level-Up Scaling', ephemeral: true });
		...
		client.messageInputHandlers.push(
			new MessageInputHandler(interaction.member.id, interaction.channelId, interaction.guildId,
				...
				async (input) => {
					...
					databaseGuild.settings.levelingSettings.levelUpScaling = input;
					...
					await interaction.followUp({ content: 'Successfully changed Level-Up Scaling', ephemeral: true });
				},
				...
				async () => {
					...
					await interaction.deleteReply();
					await interaction.followUp({ embeds: [embed], components: [rowA, rowB] });
					await interaction.followUp({ content: 'Input timed out; Level-Up Scaling has not been changed', ephemeral: true });
				},
			),
		);
	},
