...
module.exports = {
	customId: 'changelevelupthreshold',
		...
		client.messageInputHandlers.push(
			new MessageInputHandler(interaction.member.id, interaction.channelId, interaction.guildId,
				...
				async (input) => {
					...
					await interaction.followUp({ content: 'Successfully changed Level-Up Threshold', ephemeral: true });
				},
				...
				async () => {
					...
					await interaction.deleteReply();
					await interaction.followUp({ embeds: [embed], components: [rowA, rowB] });
					await interaction.followUp({ content: 'Input timed out; Level-Up Threshold has not been changed', ephemeral: true });
				},
			),
		);
	},
