...
module.exports = {
	...
	async execute(interaction, db) {
		...
		case 'set-xp-rate': {
			...
			await interaction.reply(`Successfully set the xp rate to ${guildSettings.xpForMessage}`);
			break;
		}
		case 'set-level-up-threshold': {
			guildSettings.baseLevelUpThreshold = interaction.options.getInteger('amount');
			await interaction.reply(`Successfully set the level-up threshold to ${guildSettings.baseLevelUpThreshold}`);
			break;
		}
		case 'set-level-up-scaling': {
			guildSettings.levelUpThresholdMultiplier = interaction.options.getNumber('amount');
			await interaction.reply(`Successfully set the level-up scaling to ${guildSettings.levelUpThresholdMultiplier}`);
			break;
		}
		case 'set-level-up-message-channel': {
			if (interaction.options.getChannel('channel') === null) {
				...
				await interaction.reply('Successfully set the level-up message channel to the channel of the message sent');
				break;
			}
			if (interaction.guild.members.me.permissionsIn(interaction.options.getChannel('channel')).has('SEND_MESSAGES')) {
				...
				await interaction.reply(`Successfully set the level-up message channel to <#${guildSettings.levelUpAnnouncementChannel}>`);
			}
			else {
